...
class Histogram(Density_Model):
    ...
    def get_count(self, states):
        ...
        """
        bin_name = np.apply_along_axis(self.preprocessor,
                                       axis=1,
                                       arr=states)
        counts = np.array([self.hist[_] for _ in bin_name])
        return counts
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        self.kl = self.encoder1.kl_divergence(self.prior)\
                + self.encoder2.kl_divergence(self.prior)
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        ...
        z_logstd = tf.get_variable('z_logstd', (z_size,), dtype='float32', initializer=tf.zeros_initializer)
    ...
    def get_likelihood(self, state1, state2):
        ...
        likelihood = self.sess.run(self.log_likelihood,
                                   feed_dict={self.state1: state1,
                                              self.state2: state2,
                                              self.discrim_target: np.ones(state1.shape[0])[:, None]})
        return likelihood
